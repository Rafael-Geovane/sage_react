...
export default function Home() {
  ...
  return (
    <div className="min-h-screen" style={{ background: "#0A0F16" }}>
        ...
        <div className="container relative z-10 pt-24 pb-16">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="animate-sage-fade-up">
              ...
              <p className="text-base sm:text-lg text-slate-400 mb-6 sm:mb-8 leading-relaxed max-w-lg">O colete inteligente Sage monitora sinais vitais, detecta quedas e conecta idosos aos seus cuidadores em tempo real — com tecnologia Edge AI embarcada.</p>
              <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 mb-8 sm:mb-10">
                <button onClick={handleCadastro} className="flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-4 text-white rounded-full font-semibold text-base sm:text-lg transition-all hover:-translate-y-1"
                ...
                <Link href="/loja" className="flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-4 text-white rounded-full font-semibold text-base sm:text-lg transition-all hover:bg-white/5" style={{ border: "1px solid rgba(255,255,255,0.1)" }}>
                  Ver Planos
                </Link>
              </div>
              <div className="grid grid-cols-3 gap-4 sm:gap-8">
                {[{v:98,s:"%",l:"Precisão de detecção"},{v:30,s:"s",l:"Tempo de resposta"},{v:2500,s:"+",l:"Famílias protegidas"}].map(s => (
                  <div key={s.l}>
                    <div className="text-xl sm:text-2xl font-bold" style={{ color: "#0EA5E9" }}><AnimatedCounter target={s.v} suffix={s.s} /></div>
                    <div className="text-[10px] sm:text-xs text-slate-500">{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
            <div className="flex justify-center items-center animate-sage-float">
              <div className="relative max-w-[320px] sm:max-w-none mx-auto">
                ...
                <img src="/imagens/hero_elderly.png" alt="Idosa usando colete Sage" className="relative z-10 rounded-3xl max-h-[350px] sm:max-h-[500px] w-full object-cover shadow-2xl" style={{ border: "1px solid rgba(14,165,233,0.2)" }} />
                <div className="absolute -bottom-2 sm:-bottom-4 left-2 sm:-left-4 z-20 px-3 sm:px-4 py-2 sm:py-3 rounded-xl sm:rounded-2xl" style={{ background: "rgba(10,15,22,0.9)", border: "1px solid rgba(14,165,233,0.3)", backdropFilter: "blur(20px)" }}>
                  <div className="flex items-center gap-2 sm:gap-3">
                    <Heart className="w-4 sm:w-5 h-4 sm:h-5 animate-pulse" style={{ color: "#F43F5E" }} />
                    <div><div className="text-white font-bold text-sm sm:text-lg">72 BPM</div><div className="text-[10px] sm:text-xs text-slate-400">Freq. Cardíaca</div></div>
                  </div>
                </div>
                <div className="absolute -top-2 sm:-top-4 right-2 sm:-right-4 z-20 px-3 sm:px-4 py-2 sm:py-3 rounded-xl sm:rounded-2xl" style={{ background: "rgba(10,15,22,0.9)", border: "1px solid rgba(16,185,129,0.3)", backdropFilter: "blur(20px)" }}>
                  <div className="flex items-center gap-2 sm:gap-3">
                    <Activity className="w-4 sm:w-5 h-4 sm:h-5" style={{ color: "#10B981" }} />
                    <div><div className="text-white font-bold text-sm sm:text-lg">98%</div><div className="text-[10px] sm:text-xs text-slate-400">SpO₂</div></div>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* PROBLEMA */}
      <section id="solução" className="py-12 sm:py-24" style={{ background: "#07090E" }}>
        <div className="container">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <h2 className="text-2xl sm:text-4xl font-bold text-white mb-4 sm:mb-6 leading-tight">Cada minuto conta em uma emergência</h2>
            ...
            <div className="relative rounded-3xl overflow-hidden p-6 sm:p-8" style={{ border: "1px solid rgba(255,255,255,0.06)", background: "rgba(10,15,22,0.6)" }}>
              ...
              <div className="relative z-10 grid grid-cols-2 gap-3 sm:gap-4">
                {[{v:30,s:"%",l:"das quedas causam lesões graves",c:"#EF4444",bg:"rgba(239,68,68,0.1)",b:"rgba(239,68,68,0.2)"},
                  ...
                  <div key={stat.l} className="p-3 sm:p-6 rounded-xl sm:rounded-2xl backdrop-blur-sm" style={{ background: stat.bg, border: `1px solid ${stat.b}` }}>
                    <div className="text-2xl sm:text-4xl font-bold mb-1 sm:mb-2" style={{ color: stat.c }}><AnimatedCounter target={stat.v} suffix={stat.s} /></div>
      ...
      {/* FUNCIONALIDADES */}
      <section id="funcionalidades" className="py-12 sm:py-24" style={{ background: "#07090E" }}>
        <div className="container">
          <div className="text-center mb-8 sm:mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-4 sm:mb-6 text-sm font-medium" style={{ background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.3)", color: "#0EA5E9" }}>
              <Zap className="w-4 h-4" />Funcionalidades
            </div>
            <h2 className="text-2xl sm:text-4xl font-bold text-white mb-4">Tudo que você precisa, em um colete</h2>
            <p className="text-sm sm:text-base text-slate-400 max-w-2xl mx-auto">Tecnologia de ponta embarcada em um colete confortável e discreto, projetado para o dia a dia de idosos.</p>
          </div>
      ...
      {/* COMO FUNCIONA */}
      <section id="como-funciona" className="py-12 sm:py-24" style={{ background: "linear-gradient(180deg, #07090E 0%, #0A0F16 100%)" }}>
        <div className="container">
          <div className="text-center mb-8 sm:mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-4 sm:mb-6 text-sm font-medium" style={{ background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.3)", color: "#0EA5E9" }}>
              <Clock className="w-4 h-4" />Como Funciona
            </div>
            <h2 className="text-2xl sm:text-4xl font-bold text-white mb-4">Simples para quem usa, poderoso por dentro</h2>
          </div>
      ...
      {/* DEPOIMENTOS */}
      <section id="depoimentos" className="py-12 sm:py-24" style={{ background: "#07090E" }}>
        <div className="container">
          <div className="text-center mb-8 sm:mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-4 sm:mb-6 text-sm font-medium" style={{ background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.3)", color: "#0EA5E9" }}>
              <Users className="w-4 h-4" />Depoimentos
            </div>
            <h2 className="text-2xl sm:text-4xl font-bold text-white mb-4">Quem usa, confia</h2>
          </div>
      ...
      {/* FAQ */}
      <section id="faq" className="py-12 sm:py-24" style={{ background: "linear-gradient(180deg, #07090E 0%, #0A0F16 100%)" }}>
        <div className="container max-w-3xl mx-auto">
          <div className="text-center mb-8 sm:mb-16">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-4 sm:mb-6 text-sm font-medium" style={{ background: "rgba(14,165,233,0.1)", border: "1px solid rgba(14,165,233,0.3)", color: "#0EA5E9" }}>Perguntas Frequentes</div>
            <h2 className="text-2xl sm:text-4xl font-bold text-white mb-4">Tire suas dúvidas</h2>
          </div>
          <FAQAccordion />
        </div>
      </section>
      ...
      {/* CTA */}
      <section className="py-12 sm:py-24" style={{ background: "#07090E" }}>
        <div className="container">
          <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden p-6 sm:p-12 text-center" style={{ background: "linear-gradient(135deg, rgba(14,165,233,0.15) 0%, rgba(2,132,199,0.1) 100%)", border: "1px solid rgba(14,165,233,0.2)" }}>
            ...
            <div className="relative z-10">
              <div className="w-14 sm:w-16 h-14 sm:h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 sm:mb-6" style={{ background: "rgba(14,165,233,0.2)", border: "1px solid rgba(14,165,233,0.4)" }}>
                <Shield className="w-7 sm:w-8 h-7 sm:h-8" style={{ color: "#0EA5E9" }} />
              </div>
              <h2 className="text-2xl sm:text-4xl font-bold text-white mb-4">Pronto para proteger quem você ama?</h2>
              ...
              <div className="flex flex-col sm:flex-row flex-wrap gap-3 sm:gap-4 justify-center">
                <button onClick={handleCadastro} className="flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-4 text-white rounded-full font-semibold text-base sm:text-lg transition-all hover:-translate-y-1" style={{ background: "linear-gradient(135deg, #0EA5E9, #0284C7)", boxShadow: "0 6px 20px rgba(14,165,233,0.4)" }}>
                  Criar Conta Grátis <ArrowRight className="w-5 h-5" />
                </button>
                <Link href="/loja" className="flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-4 text-white rounded-full font-semibold text-base sm:text-lg transition-all hover:bg-white/5" style={{ border: "1px solid rgba(255,255,255,0.15)" }}>
                  Ver Planos e Preços
                </Link>
              </div>
